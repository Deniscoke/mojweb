 ...
 *
 * Every `alt` below describes what is actually visible in that specific file.
 * None of it is derived from a filename, and nothing here describes an image
 * that does not exist. Assets whose provenance could not be confirmed, or that
 * would need redaction, are not listed — see `docs/assets-found.md`.
 */
...
const EVIDENCE = path.resolve(process.cwd(), '..', 'artifacts', 'evidence-ready')

type Kind = 'screenshot' | 'diagram' | 'photo' | 'generated' | 'video'
...
interface MediaSpec {
  ...
  credit?: string
  /** Project slug this belongs to, in intended display order. */
  project: string
  order: number
}
...
async function main() {
  ...
  console.log(`\n[media] Importing ${mediaSpecs.length} evidence files from ${EVIDENCE}`)
  ...
  for (const spec of mediaSpecs) {
    const filePath = path.join(EVIDENCE, spec.file)
